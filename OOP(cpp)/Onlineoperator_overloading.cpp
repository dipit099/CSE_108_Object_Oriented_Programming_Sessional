...
public:
    ...
    double getRadius();
    void print();
    void reset() { center.setX(0); center.setY(0); radius=0; }
    ~Circle();

    // your code
    friend RightCircularCone operator*(Circle c1 , RightCircularCone r1);
};
...
public:
    ...
    // your code
    friend RightCircularCone operator*(Circle c1 , int num);
    friend RightCircularCone operator*(Circle c1 , RightCircularCone r1);
};
// your code
RightCircularCone operator*(Circle c1 , int num)
{
    RightCircularCone temp;
    temp.setBase(c1);
    temp.volume =  (pi * c1.getRadius()*c1.getRadius()* num)/3;
   //cout<< num << endl;
    return temp;
}

RightCircularCone operator*(Circle c1 , RightCircularCone r1)
{
    RightCircularCone temp;
    temp.setBase(c1);
    Circle for_radius = r1.getBase();
    double radius = for_radius.radius;
    double height =( r1.getVolume()*3)/((pi * radius* radius));

    temp.volume =  (pi * c1.getRadius()*c1.getRadius()* height)/3;
    //cout<< endl;
    //cout << r1.getVolume()<< ' '<<' ' <<radius<< endl;
    return temp;
}
...
int main()
{
    ...
    printVar("Initial c1"); c1.print();     // Initial c1 = [Center: (5,10), Radius: 5]
    printVar("Initial c2"); c2.print();     // Initial c2 = [Center: (15,10), Radius: 7]

    RightCircularCone r1, r2;
    printVar("Initial r1"); r1.print();     // Initial r1 = {Base = [Center: (0,0), Radius: 0], Volume = -1}
    printVar("Initial r2"); r2.print();     // Initial r2 = {Base = [Center: (0,0), Radius: 0], Volume = -1}
    ...
    r1 = c1 * 10;
    printVar("Final r1"); r1.print();       // Final r1 = {Base = [Center: (5,10), Radius: 5], Volume = 261.8}
    ...
    r2 = c2 * r1;
    printVar("Final r2"); r2.print();       // Final r2 = {Base = [Center: (15,10), Radius: 7], Volume = 513.128}
