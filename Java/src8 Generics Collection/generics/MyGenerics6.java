...
class Stats<T extends Number> {
    ...
    double average() {
        ...
        for (int i = 0; i < nums.length; i++) {
            sum += nums[i].doubleValue(); // use double.Value() method of Number
        }
...
//s1 call korbe so sameAvg ta s1 er type diye bounder hbe so then cant use Double type anymore..problem so use wildcard
    }
}
